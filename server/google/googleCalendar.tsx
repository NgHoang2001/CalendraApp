...
async function getOAuthClient(clerkUserId: string) {
    try {
        ...
        if (data.length === 0 || !data[0].token) {
            throw new Error("Không tìm thấy dữ liệu hoặc mã thông báo OAuth cho người dùng.")
        }
        ...
        throw new Error(`Không nhận được khách hàng OAuth: ${err.message}`)
    }
}

// tìm nạp và định dạng các sự kiện lịch cho người dùng giữa phạm vi ngày nhất định
export async function getCalendarEventTimes(
    ...
    try {
        ...
        if (!oAuthClient) {
            throw new Error("Khách hàng OAuth không thể có được.")
        }
        ...
        throw new Error(`Không thể tìm nạp các sự kiện lịch: ${err.message || err}`)
