...
// Chức năng này lấy lịch trình (và khả năng của nó) cho một người dùng nhất định từ cơ sở dữ liệu
...
// Hành động máy chủ này lưu lịch trình và khả năng của người dùng
export async function saveSchedule(unsafeData: z.infer<typeof scheduleFormSchema>) {// Accepts unvalidated form data
    try {
        ...
        if (!success || !userId) {
            throw new Error("Dữ liệu không đúng định dạng hoặc người dùng chưa đăng nhập.")
        }
        ...
        throw new Error(`Lưu trữ lịch thất bại: ${error.message || error}`)
    } finally {
...
/**
 * Lọc danh sách các khe thời gian để chỉ trả về những cái đó:
 * 1. Kết hợp lịch trình khả dụng của người dùng
 * 2. Không trùng với các sự kiện Lịch Google hiện có
 */
export async function getValidTimesFromSchedule(
    ...
    // Xác định sự khởi đầu và kết thúc của toàn bộ phạm vi để kiểm tra
    ...
    // Tìm nạp tất cả các sự kiện Lịch hiện có của Google giữa bắt đầu và kết thúc
